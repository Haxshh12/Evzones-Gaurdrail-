...
# Fix for Windows compatibility - resource module is Unix-only
try:
    import resource
except ImportError:
    resource = None
...
def set_file_limit() -> None:
    ...
    if resource is None:
        # Windows doesn't have resource limits, so we skip this
        logger.debug("Skipping file limit setting on Windows")
        return
