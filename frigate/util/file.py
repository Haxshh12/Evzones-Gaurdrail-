...
# Fix for Windows compatibility - use appropriate locking mechanism
try:
    import fcntl
    HAS_FCNTL = True
except ImportError:
...
class FileLock:
            ...
            while time.time() - start_time < timeout:
                try:
                    if HAS_FCNTL:
                        ...
                        # No locking available, just pretend we got the lock
                        logger.warning("No file locking available, using fake lock")
    ...
    def release(self) -> None:
        ...
        try:
            # Close file descriptor and release lock
            if self._fd is not None:
                try:
                    if HAS_FCNTL:
