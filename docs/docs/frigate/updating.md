...
The current stable version of Harshjjj NVR is **0.18.0**.
...
- **Check Release Notes**: Carefully review the release notes for breaking changes or configuration updates.
...
2. **Update and Pull the Latest Image**:
   - If using Docker Compose:
     - Edit your `docker-compose.yml` file to specify the desired version tag (e.g., `0.17.0` instead of `0.16.4`). For example:
       ...
       services:
         frigate:
           image: harshjjj/nvr:0.18.0
       ```
     - Then pull the image:
       ```bash
       docker pull harshjjj/nvr:0.18.0
       ```
     - **Note for `stable` Tag Users**: If your `docker-compose.yml` uses the `stable` tag (e.g., `harshjjj/nvr:stable`), you don't need to update the tag manually. The `stable` tag always points to the latest stable release after pulling.
   - If using `docker run`:
...
- Ensure your `/config/config.yml` is compatible with the new version.
...
3. Revert to the previous image version:
   - For Docker: Specify an older tag (e.g., `harshjjj/nvr:0.16.4`) in your `docker run` command.
   - For Docker Compose: Edit your `docker-compose.yml`, specify the older version tag (e.g., `harshjjj/nvr:0.16.4`), and re-run `docker compose up -d`.
...
Common questions can be found in the support documentation.
